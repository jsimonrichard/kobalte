...
// or
import { Root, Label, ... } from "@trellis-app/kobalte-core/text-field";
// or (deprecated)
import { TextField } from "@trellis-app/kobalte-core";
```
...
`TextField` is equivalent to the `Root` import from `@trellis-app/kobalte-core/text-field` (and deprecated `TextField.Root`).
