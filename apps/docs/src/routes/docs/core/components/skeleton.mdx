...
// or
import { Root } from "@trellis-app/kobalte-core/skeleton";
// or (deprecated)
import { Skeleton } from "@trellis-app/kobalte-core";
```
...
`Skeleton` is equivalent to the `Root` import from `@trellis-app/kobalte-core/skeleton` (and deprecated `Skeleton.Root`).
