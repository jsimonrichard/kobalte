...
// or
import { Root, Trigger, ... } from "@trellis-app/kobalte-core/hover-card";
// or (deprecated)
import { HoverCard } from "@trellis-app/kobalte-core";
```
...
`HoverCard` is equivalent to the `Root` import from `@trellis-app/kobalte-core/hover-card` (and deprecated `HoverCard.Root`).
