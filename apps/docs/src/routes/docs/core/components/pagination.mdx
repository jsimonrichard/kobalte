...
// or
import { Root, Item, ... } from "@trellis-app/kobalte-core/pagination";
// or (deprecated)
import { Pagination } from "@trellis-app/kobalte-core";
```
...
`Pagination` is equivalent to the `Root` import from `@trellis-app/kobalte-core/pagination` (and deprecated `Pagination.Root`).
