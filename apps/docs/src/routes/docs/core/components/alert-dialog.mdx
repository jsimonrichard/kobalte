...
// or
import { Root, Trigger, ... } from "@trellis-app/kobalte-core/alert-dialog";
// or (deprecated)
import { AlertDialog } from "@trellis-app/kobalte-core";
```
...
`AlertDialog` is equivalent to the `Root` import from `@trellis-app/kobalte-core/alert-dialog` (and deprecated `AlertDialog.Root`).
