...
// or
import { Root, Input, ... } from "@trellis-app/kobalte-core/checkbox";
// or (deprecated)
import { Checkbox } from "@trellis-app/kobalte-core";
```
...
`Checkbox` is equivalent to the `Root` import from `@trellis-app/kobalte-core/checkbox` (and deprecated `Checkbox.Root`).
