...
// or
import { Root, Label, ... } from "@trellis-app/kobalte-core/select";
// or (deprecated)
import { Select } from "@trellis-app/kobalte-core";
```
...
`Select` is equivalent to the `Root` import from `@trellis-app/kobalte-core/select` (and deprecated `Select.Root`).
