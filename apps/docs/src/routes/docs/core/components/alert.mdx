import { Preview, TabsSnippets } from "../../../../components";
import { BasicExample } from "../../../../examples/alert";

# Alert

Display a brief, important message in a way that attracts the user's attention without interrupting the user's task.

## Import

```ts
import { Alert } from "@trellis-app/kobalte-core/alert";
// or
import { Root } from "@trellis-app/kobalte-core/alert";
// or (deprecated)
import { Alert } from "@trellis-app/kobalte-core";
```

## Features

- Adhere to the [WAI ARIA Alert](https://www.w3.org/WAI/ARIA/apg/patterns/alert/) design pattern.

## Anatomy

The alert consists of :

- **Alert:** The root container for an alert.

```tsx
<Alert />
```

## Example

<Preview>
	<BasicExample />
</Preview>

<TabsSnippets>
  <TabsSnippets.List>
    <TabsSnippets.Trigger value="index.tsx">index.tsx</TabsSnippets.Trigger>
    <TabsSnippets.Trigger value="style.css">style.css</TabsSnippets.Trigger>
  </TabsSnippets.List>
{/* <!-- prettier-ignore-start --> */}
  <TabsSnippets.Content value="index.tsx">
    ```tsx
    import { Alert } from "@trellis-app/kobalte-core/alert";
    import "./style.css";

    function App() {
      return <Alert class="alert">Kobalte is going live soon, get ready!</Alert>;
    }
    ```

  </TabsSnippets.Content>
  <TabsSnippets.Content value="style.css">
    ```css
    .alert {
      border-radius: 6px;
      padding: 12px 16px;
      background-color: hsl(204 100% 97%);
      color: hsl(201 96% 32%);
      font-size: 16px;
    }
    ```
  </TabsSnippets.Content>
{/* <!-- prettier-ignore-end -->*/}
</TabsSnippets>

## API reference

### Alert

`Alert` is equivalent to the `Root` import from `@trellis-app/kobalte-core/alert` (and deprecated `Alert.Root`).

Renders a `div` by default and support all its props.

## Rendered elements

| Component | Default rendered element |
| :-------- | :----------------------- |
| `Alert`   | `div`                    |
