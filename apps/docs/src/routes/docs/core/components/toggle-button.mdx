...
// or
import { Root } from "@trellis-app/kobalte-core/toggle-button";
// or (deprecated)
import { ToggleButton } from "@trellis-app/kobalte-core";
```
...
`ToggleButton` is equivalent to the `Root` import from `@trellis-app/kobalte-core/toggle-button` (and deprecated `ToggleButton.Root`).
