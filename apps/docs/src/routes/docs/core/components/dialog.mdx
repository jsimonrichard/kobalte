...
// or
import { Root, Trigger, ... } from "@trellis-app/kobalte-core/dialog";
// or (deprecated)
import { Dialog } from "@trellis-app/kobalte-core";
```
...
`Dialog` is equivalent to the `Root` import from `@trellis-app/kobalte-core/dialog` (and deprecated `Dialog.Root`).
