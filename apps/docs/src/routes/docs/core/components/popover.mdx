...
// or
import { Root, Trigger, ... } from "@trellis-app/kobalte-core/popover";
// or (deprecated)
...
`Popover` is equivalent to the `Root` import from `@trellis-app/kobalte-core/popover` (and deprecated `Popover.Root`).
