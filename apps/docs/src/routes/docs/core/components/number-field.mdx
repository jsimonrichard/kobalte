...
// or
import { Root, Label, ... } from "@trellis-app/kobalte-core/number-field";
// or (deprecated)
import { NumberField } from "@trellis-app/kobalte-core";
```
...
`NumberField` is equivalent to the `Root` import from `@trellis-app/kobalte-core/number-field` (and deprecated `NumberField.Root`).
