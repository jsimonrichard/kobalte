...
// or
import { Root, Img, ... } from "@trellis-app/kobalte-core/image";
// or (deprecated)
import { Image } from "@trellis-app/kobalte-core";
```
...
`Image` is equivalent to the `Root` import from `@trellis-app/kobalte-core/image` (and deprecated `Image.Root`).
