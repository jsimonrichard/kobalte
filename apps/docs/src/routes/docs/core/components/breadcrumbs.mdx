...
// or
import { Root, Link, ... } from "@trellis-app/kobalte-core/breadcrumbs";
// or (deprecated)
import { Breadcrumbs } from "@trellis-app/kobalte-core";
```
...
`Breadcrumbs` is equivalent to the `Root` import from `@trellis-app/kobalte-core/breadcrumbs` (and deprecated `Breadcrumbs.Root`).
