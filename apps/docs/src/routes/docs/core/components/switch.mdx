...
// or
import { Root, Input, ... } from "@trellis-app/kobalte-core/switch";
// or (deprecated)
import { Switch } from "@trellis-app/kobalte-core";
```
...
`Switch` is equivalent to the `Root` import from `@trellis-app/kobalte-core/switch` (and deprecated `Switch.Root`).
