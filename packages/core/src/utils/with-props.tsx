...
export type PropsWithout<T extends ValidComponent, U extends ValidComponent, ExtraProps extends {} = {}> = Omit<ComponentProps<T>, keyof ExtraProps> &
Partial<ComponentProps<T>> & {
    ...
    if (extraProps instanceof Function) {
      ...
      // filter props
			filteredProps = Object.keys(props)
				.filter((key) => !excludeFromComponentProps
				.includes(key as keyof ExtraPropInputs))
				.reduce((newObj: PropsWithout<T, U ,ExtraProps>, key) => { // combine same keys
